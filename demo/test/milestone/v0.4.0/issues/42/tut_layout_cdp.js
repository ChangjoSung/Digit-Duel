/* #42 튜토리얼 카드 레이아웃 실측 — 헤드리스 Chrome(CDP)으로 demo/index.html을 열고 10단계 전부를 여러 뷰포트에서 측정한다.
   사용: node demo/test/milestone/v0.4.0/issues/42/tut_layout_cdp.js [--html <index.html>] [--ref <commit>] [--out <dir>] [--chrome <chrome.exe>] [--shots] [--read-only]
...
const READ_ONLY=args.includes("--read-only"), REF=opt("--ref",null);
const OUT=opt("--out",path.join(os.tmpdir(),"tut_layout_cdp")), SHOTS=args.includes("--shots")&&!READ_ONLY;
...
const SOURCE=REF?execFileSync("git",["show",REF+":demo/index.html"],{cwd:path.resolve(__dirname,"../../../../../.."),maxBuffer:64*1024*1024}).toString("utf8"):fs.readFileSync(HTML,"utf8");
console.log("SOURCE "+(REF||HTML)+" sha256="+crypto.createHash("sha256").update(SOURCE).digest("hex"));
const VIEWPORTS=[
...
const MEASURE=`(()=>{
  ...
  const cards=[...body.querySelectorAll(".tut-card")], boxR=R(box), issues=[], glyphs=[];
  ...
  cards.forEach((c,k)=>{ const cr=R(c);
    ...
    // #114 Saturn: DOM boxes do not reveal clipped SVG glyphs. Map each text's actual bbox into its root viewBox.
    c.querySelectorAll("svg").forEach((svg,si)=>{const vb=svg.viewBox.baseVal, texts=[];
      svg.querySelectorAll("text").forEach((el,ti)=>{const b=el.getBBox(),m=svg.getScreenCTM().inverse().multiply(el.getScreenCTM());
        const pts=[[b.x,b.y],[b.x+b.width,b.y],[b.x,b.y+b.height],[b.x+b.width,b.y+b.height]].map(([x,y])=>new DOMPoint(x,y).matrixTransform(m));
        const g={card:k+1,svg:si,text:ti,glyph:el.textContent,left:Math.min(...pts.map(p=>p.x)),top:Math.min(...pts.map(p=>p.y)),right:Math.max(...pts.map(p=>p.x)),bottom:Math.max(...pts.map(p=>p.y))};
        glyphs.push(g);texts.push(g);
        if(g.left<vb.x-0.001||g.top<vb.y-0.001||g.right>vb.x+vb.width+0.001||g.bottom>vb.y+vb.height+0.001) issues.push("SVG glyph clipped card"+(k+1)+" svg"+si+" text"+ti+" "+g.glyph+" bbox="+[g.left,g.top,g.right,g.bottom].map(n=>n.toFixed(3)).join(","));
      });
      for(let i=0;i<texts.length;i++)for(let j=i+1;j<texts.length;j++)if(ov(texts[i],texts[j]))issues.push("SVG glyph overlap card"+(k+1)+" svg"+si+" text"+i+"/"+j);
    });
  });
  ...
  return {cols:lefts.length,cards:cards.length,pageHscroll,boxW:Math.round(boxR.width),boxH:Math.round(boxR.height),vscroll:box.scrollHeight>box.clientHeight+1?box.scrollHeight+">"+box.clientHeight:"",
    scrollTop,pageTop,titleVis,head1Vis,card1Vis,focusOn,glyphs,
...
function launch(){
  return new Promise((res,rej)=>{
    ...
    const p=spawn(CHROME,["--headless=new","--remote-debugging-port=0","--user-data-dir="+udd,"--no-first-run","--no-default-browser-check","--disable-gpu","--hide-scrollbars","--allow-file-access-from-files","--lang=ko-KR","about:blank"],{windowsHide:true,stdio:["ignore","pipe","pipe"]});
...
(async()=>{
  ...
  try{
    ...
    for(const vp of VIEWPORTS){
      ...
      await cdp.send("Page.navigate",{url:"about:blank"},sid); await loaded; // fresh JS realm avoids redeclaring product const bindings
      const {frameTree}=await cdp.send("Page.getFrameTree",{},sid);
      await cdp.send("Page.setDocumentContent",{frameId:frameTree.frame.id,html:SOURCE},sid); // freeze current or git-ref source in memory; no HTML copy
  ...
  } finally { try{proc.kill();}catch(e){} await new Promise(r=>setTimeout(r,500));
    const exact=path.resolve(udd);if(path.dirname(exact)!==path.resolve(os.tmpdir())||!path.basename(exact).startsWith("tutcdp-"))throw Error("Unexpected owned profile path");
    fs.rmSync(exact,{recursive:true,force:true});console.log("CLEANUP owned PID="+proc.pid+" profile="+exact); }
  if(!READ_ONLY) fs.writeFileSync(path.join(OUT,"tut_layout_report.json"),JSON.stringify(report,null,1));
  ...
  console.log(`\n=== tut_layout_cdp: ${report.length} 측정 · 문제 ${bad}건 · ${READ_ONLY?"read-only (no artifacts)":"보고서 "+path.join(OUT,"tut_layout_report.json")}${SHOTS?" · 스크린샷 "+OUT:""}`);
