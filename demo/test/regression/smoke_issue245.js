...
/* #263 T3: forcedExempt 발생 지점이 3 → 4 곳이 됐다 — drainForced 의 세 가지(말 소멸·대상 재검사 실패·전투 횟수 소진)에
   더해 forcedAuto(대상 선택 30초 만료)가 "적격 대상이 남지 않았다"를 같은 어휘로 알린다. 면제 사유를 조용히 누락하지
   않는다는 #18 계약은 그대로이고, 개시 헬퍼(forcedContactStart)는 여전히 한 곳이다. */
ok(/function drainForcedQueue\(autoStart\)\{\s*const result=dispatchCoreAction\(\{t:"drainForced",autoStart:!!autoStart\}\);/.test(T.html)&&(T.html.match(/type:"forcedExempt"/g)||[]).length===4&&(T.html.match(/forcedContactStart\(event\.piece,event\.list\)/g)||[]).length===1,"AI, end turn, flee and battle completion share one Core drain entry point and the promoted battle reuses the shared display helper");
