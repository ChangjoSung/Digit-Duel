/* #96 감전 부여 확률 하향 (v0.4.4 §4) 헤드리스 회귀 — node demo/test/regression/smoke_shock.js [demo/index.html]
...
/* ===== H. 테스트 결정론 계약 (AC 7): statusProb=1 을 고정하는 스위트는 shockProb=1 도 고정 ===== */
{
  ...
  // H0 (#134): 이 계약이 실제로 지켜야 할 스위트 — statusProb=1 을 고정하는 파일들 — 이 스캔 대상에 남아 있는지 먼저 본다.
  // 이 목록이 비거나 줄면 H1 은 검사할 게 없어져 조용히 통과한다. 파일 개수를 세는 게 아니라 이름을 지목한다.
  const REQUIRED=["smoke_cycle5.js","smoke_minion_art.js","smoke_attack_balance.js","smoke_turnflow.js","smoke_turnflow_timers.js"];
  const missing=REQUIRED.filter(f=>!files.includes(f));
  ok(missing.length===0,"H0 statusProb=1 고정 스위트 "+REQUIRED.length+"종이 모두 스캔 대상 ["+(missing.join(", ")||"누락 0")+"]");
  const notFixing=REQUIRED.filter(f=>files.includes(f)&&!/statusProb\s*=\s*1\b/.test(readSrc(f)));
  ok(notFixing.length===0,"H0b 지목한 스위트가 실제로 statusProb=1 을 고정 ["+(notFixing.join(", ")||"이탈 0")+"]");
