...
/* 하네스 els는 getElementById로 접근한 순간 생기므로 document를 거쳐 집는다 (렌더 문자열만으로는 아직 없음) */
function $el(id){ return global.document.getElementById(id); }
function toasts(){ return ($el("toasts").children||[]).map(x=>x.textContent); }
...
/* ===== A. 서버 주소 기본값 — 페이지 프로토콜·저장값 ===== */
{
  ...
  ok(global.location.protocol==="file:"&&global.location.host==="","A2 하네스 location 스텁은 실제 file:// 처럼 protocol·host를 갖는다 (없으면 로드 즉시 예외)");
  ok($el("sidePanel").innerHTML.indexOf(T.NET_LAN_DEFAULT)>=0,"A3 메뉴 서버 주소 입력칸에 기본값이 채워진다 (렌더가 예외 없이 완주)");
}
{
...
}
{
  ...
  T.netConnect();
  ok(T.wsLog.length===1&&/이미 서버에 연결 중/.test(toasts().join("|")),"B3 이미 연결 중이면 소켓을 새로 만들지 않는다");
}
{
  const T=connectWith(FILE_HREF,"1.2.3.4");
  ok(T.wsLog.length===1&&T.wsLog[0].url==="ws://1.2.3.4:8080","B4 포트 생략 → 기본 포트 8080 보정");
  ok(global.localStorage.getItem("netServer")==="1.2.3.4:8080","B5 저장되는 값은 원문이 아니라 정규화 결과");
}
{
...
}
{
  ...
  ok(T.wsLog[0].url==="ws://192.168.5.5:7000"&&global.localStorage.getItem("netServer")==="192.168.5.5:7000","B7 앞뒤 공백 제거 + ws:// 스킴 제거");
}
{
...
}
{
  ...
  $el("netServer").value="1.2.3.4:5555";
...
function prepared(){ const T=loadAt(FILE_HREF); T.netPrepare(); return T; }
{
  ...
  ok(/온라인 대전 — 내 로스터 선택/.test($el("sidePanel").innerHTML)&&/배치 완료 → 매칭 시작/.test($el("sidePanel").innerHTML),"C3 배치 화면 안내·버튼 문구가 온라인용");
}
{
  const T=prepared();
  T.netAction({t:"setupDone"}); // 로스터 0종·배치 0개 상태
  ok(T.NET.queued===false&&T.NET.mySetup===null&&T.wsLog.length===0&&/로스터 6종 선택과 14개 배치/.test(toasts().join("|")),"C4 배치 미완 상태에서는 매칭 큐에 들어가지 않는다 (접속 시도 0)");
  ...
  ok(/매칭 대기 중/.test($el("sidePanel").innerHTML),"C9 큐 진입 후 화면은 매칭 대기");
...
}
{
  ...
  T.netPrepare();
  ok(T.wsLog.length===1&&/이미 온라인 대전이 진행 중/.test(toasts().join("|")),"C12 이미 매칭 중이면 netPrepare가 배치를 초기화하지 않는다");
}
{
