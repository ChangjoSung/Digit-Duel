...
rem Digit Dual public match server (#217 server authority) - Windows one-click launcher (LOCAL ONLY).
rem
rem This launcher has exactly one purpose: start the public-lobby server bound to this PC.
rem It reads no caller-supplied argument at all - no positional token, no
rem argument list - never prints one, and never forwards one to the server.
...
rem separate server: 서버시작.bat (port 8080). For LAN play use 공개LAN서버시작.bat.

title Digit Dual Public Server - Local only
