...
const MUTATIONS = [
  ...
  { g: 1, file: "core.js", contract: "__recruitCore 에 숫자 토큰을 넘김",
  ...
  { g: 1, file: "ui.js", contract: "이벤트 소비 칸 오타 (event.slot→event.slto)",
    from: 'if(event.type==="battleSlot"){ execSlot(event.side,event.slot); continue; }',
    to:   'if(event.type==="battleSlot"){ execSlot(event.side,event.slto); continue; }' },
  ...
  { g: 1, file: "data.js", contract: "전투 상태에 없는 칸 읽기 (B.maxRounds→B.maxRoundz)",
    from: 'function battleMaxRounds(){ const B=S.battle; return (B&&B.maxRounds)||BAL.maxRounds; }',
    to:   'function battleMaxRounds(){ const B=S.battle; return (B&&B.maxRoundz)||BAL.maxRounds; }' },
