/* #105 — tools/media/readme_media_capture.js 의 READ_ONLY 보증을 도구 바깥에서 관측하는 하네스 (Saturn 재검증용).
   도구 내부 WRITE-CHECK 를 믿지 않고, 이 프로세스가 독립적으로 저장소·tmp 스냅샷을 찍어 전후를 비교하고,
   소스를 변조한 사본으로 정적 검사·런타임 게이트가 실제로 위반을 잡는지(음성 대조) 확인한다.

   사용: node tools/media/test/readme_media_readonly_test.js [--quick] [--no-gh]
...
const ROOT=path.resolve(__dirname,"..","..",".."), TOOL=path.join(ROOT,"tools","media","readme_media_capture.js");
...
const neg=fs.mkdtempSync(path.join(os.tmpdir(),"rmc-negtest-")); fs.mkdirSync(path.join(neg,"tools","media"),{recursive:true});
try{
  ...
  const a=path.join(neg,"tools","media","inject_static.js"); fs.writeFileSync(a,src.replace(marker,'  fs.writeFileSync(path.join(ROOT,"STRAY.txt"),"x");\n'+marker));
  ...
  const b=path.join(neg,"tools","media","inject_runtime.js"); fs.writeFileSync(b,src.replace(marker,'  fs["write"+"FileSync"](path.join(ROOT,"STRAY.txt"),"x");\n'+marker));
  ...
  const c=path.join(neg,"tools","media","inject_mkdtemp.js"); fs.writeFileSync(c,src.replace(marker,'  fs["mkdtemp"+"Sync"](path.join(os.tmpdir(),"readme-media-src-"));\n'+marker));
