/* #105 README 미디어 — 출시 태그의 실제 게임 화면(튜토리얼 10단계)을 헤드리스 Chrome(CDP)으로 캡처하고, README 의 로컬 이미지·링크와
   ...
   사용:
     node tools/readme_media_capture.js capture [--ref v0.4.3] [--out docs/media] [--viewport 1280x900] [--dpr 2] [--chrome <chrome.exe>]
     ...
     node tools/readme_media_capture.js verify --read-only [--viewport 1100x900|390x844] [--no-gh] [--no-render]
       - Saturn 독립 재검증용. 위 verify 와 같은 검사(헤드리스 Chrome 실제 렌더 포함)를 수행하되 stdout 으로만 보고한다 — HTML·PNG·JSON 어느 것도 저장하지 않는다.
     node tools/readme_media_capture.js capture --read-only [--allow-hash-diff]
...
/* ── 헤드리스 Chrome + CDP (demo/test/tut_layout_cdp.js 의 패턴을 재사용) ───────────────────────────────── */
