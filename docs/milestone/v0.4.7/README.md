...
## 이어서 착수한 범위 — #146 · #131 · #130 (종결) · #128 (새 CJ QA 대기)

2026-09-10 CJ가 도망 재설계·행동 없음 처리·텔레포트 제외·보호막 합산·튜토리얼 표시를 승인해 네 Issue를 함께 착수했다. **#146 · #131 · #130은 [#146](https://github.com/ChangjoSung/Digit-Duel/issues/146)을 기준으로 [통합 PR152](https://github.com/ChangjoSung/Digit-Duel/pull/152)에서 관리해 2026-09-10 CJ 플레이 QA PASS로 CLOSED**했으며, 그 범위의 제품 코어·헤드리스·미디어·최종 README 렌더 Saturn PASS와 Chrome 81/0은 당시 검수 근거다. PR152는 그 세 Issue의 이력이다. **#128은 후속 CJ 승인으로 매 페이지 로드 표시를 구현했고 새 CJ 플레이 QA 대기로 OPEN이다.** 제품·Chrome 증빙은 Saturn PASS이며 최종 PR·CI·dev 통합은 [#128 통합 기록](issues/128/Mercury/report.md)을 따른다. 진행 상태의 최종 확정은 각 Issue와 그 Issue에 연결되는 PR이 원본이다. #128의 최초 신고 미재현과 후속 Worker 정리 절차 위반은 [PD 기록](issues/146/Mercury/report.md)에 작성 당시 그대로 보존한다. v0.4.7은 아직 미출시다.
...
| [#128](https://github.com/ChangjoSung/Digit-Duel/issues/128) | 튜토리얼 표시 — 매 페이지 로드 자동 표시로 정책 변경(구현·새 CJ QA 대기) | Venus(사용자 문서) · Mars(구현) | 5 · [문서 동기화](issues/128/Venus/docs-report.md) |
...
이후 **CJ가 준 상세 증상은 같은 브라우저·같은 주소 재방문에서 자동 표시를 생략하는 기존 `localStorage` 정책으로 설명**되며, 그래서 원인 추적을 더 밀지 않고 **매 페이지 로드 표시로 정책을 바꾸는 쪽이 승인**됐다(2026-09-10, GDD-13 DL47). 이번 정책 변경은 처음 신고를 재현했거나 해결했다는 뜻으로 소급하지 않는다. 이번 구현의 HTML blob은 `8027cd72d8445c9a7077d0e43b1c5559645ee165`이며 제품·실브라우저 독립 검수 후 새 CJ QA를 기다린다. PR·CI·dev 통합은 [통합 기록](issues/128/Mercury/report.md)과 Issue128 연결 PR이 원본이다.
...
| CJ 플레이 QA | #146 · #131 · #130 **PASS (2026-09-10)** — CLOSED / #128 **대기** — 매 페이지 로드 표시 구현, OPEN 유지 |
| 릴리스 | 미출시 — 정식 v0.4.5 유지 |
...
위 표는 아직 착수하지 않은 Issue만 남긴 것이다. 종결한 #121 · #125 · #129 · #146 · #131 · #130과 구현 후 새 CJ QA를 기다리는 #128은 위 두 절에 있다. 소관은 [CLAUDE.md](../../../CLAUDE.md)의 역할 계약(영역 → 역할)에서 따온 것이며, 실제 배치는 PD 라우팅으로 확정된다. 게임 규칙을 바꾸는 나머지 항목(#119 · #120 · #126)은 CJ 승인 전까지 규격이 확정되지 않는다.
