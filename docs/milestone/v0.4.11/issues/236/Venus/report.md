...
- 표기: **[확정]** = GDD-23/24에 CJ 결정으로 적힌 것 · **[현행]** = 지금 코드에서 확인한 사실 · **[추론]** = 확정 문장에서 Venus가 끌어낸 해석(이의 없으면 기본값) · **[제안]** = Venus 권장, 결정 아님 · **[기획 필요]** = 규칙 없음, 결정 요청
...
```mermaid
flowchart TD
  S01["S01 경기 시작 상점<br/>🪙10 · ⭐1 미보유 종 5칸 · 무료 보충<br/>필드 6칸 먼저 → 가방"] -->|"완료(필드 6칸) 또는 시간 초과 자동 구매"| S02["S02 비공개 배치 (현행)"]
  S02 --> P["보드 턴 진행"]
...
**용어** — *진열*: 상점에 뜬 상품 5칸. *차액 승급*: 가진 종의 더 높은 등급을 (진열 등급가 − 내 등급가)만 내고 사는 것. *동급 합성*: 같은 등급을 정가에 사서 한 등급 올리는 것. *원장(ledger)*: 그 개체에 실제로 낸 코인 합계, 판매가와 같다. *예비 재화 규칙*: 시작 상점에서 필드 빈칸 k개일 때, 칸을 채우지 않는 지출 뒤에도 🪙k 이상 남아야 하는 규칙 — 6명 구매를 보장하는 안전장치. *가림*: 핫시트에서 다음 사람에게 기기를 넘길 때 앞 사람 화면을 덮는 창. *공정 관측*: AI가 사람이 볼 수 있는 정보만 쓰는 원칙.

## 4. 해 보기 — 한 개체의 코인 일생

1. S01: 🪙10. 필드가 비어 k=6이므로 볼·새로 고침 같은 지출은 최대 🪙4. 물 여우 ⭐1을 🪙1에 산다 → 원장 1, 필드 1칸.
...
"PD 수용"은 [Mars 보고](../Mars/report.md) 4장에 적힌 상태를 옮긴 것이며, Venus가 PD 결정을 따로 확인하지 않았다. **CJ 결정으로 바뀐 항목은 D1 하나다(2026-09-24).**
...
| AC02 | S01 진열 5칸 = 서로 다른 미보유 ⭐1. 구매 즉시 그 칸 무료 보충, 새 칸도 미보유 ⭐1 | E2 | C |
| AC03 | S01 구매 순서: 1~6번째 필드, 7번째부터 가방. 가방 3칸 초과 신규 구매 거부 | E2·E6 | C |
| AC04 | 시작 🪙10·k=6 → 여유(🪙 − k) = 4. 볼 4개 구매 → 🪙6·k=6·여유 0, 5번째 볼은 지출 뒤 🪙5 < 6이라 거부. 하수인 1명 구매는 🪙와 k를 함께 1 줄여 여유를 바꾸지 않는다 — 그 뒤(🪙5·k=5)에도 볼·새로 고침 거부. 순서와 무관하게 S01의 칸을 채우지 않는 지출 합계는 최대 🪙4. 새로 고침도 같은 규칙 | E2 | C |
...
| AC08 | S01 시간 초과(필드 2칸) → 진열 ①부터 자동 구매해 6칸, 확정 거래 보존, 미확정 모달 취소 | E2·E3 | C |
