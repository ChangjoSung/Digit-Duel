...
## 10. 후속 — PR239 CI A 실패 수정 (2026-09-16, `ctx_2d8c814c4ffe`)

`smoke_turnflow.js` D6 이 CI(run 35067901329 / job 104702302213, commit `9336afd`)에서만 202 pass 1 fail 로 떨어졌다. **로컬에서는 재현되지 않았다** — 아래 확률 때문이다.

### 근본 원인 — #233 이 바꾼 전제 두 가지가 픽스처에 남아 있었다

D6 은 `execSlot("A",0)` 한 방 뒤 `S().battle.recA>0`(누적 유효 피해 지표가 계속 기록되는가)만 본다. 그런데 #233 이 **회피(💨)를 실제 스탯으로 만들면서** 전제가 깨졌다:

- 방어측 회피율은 **새 게임마다 무작위 배정되는 아키타입**에서 온다. 40회 표본 실측 draw: `0% → 24회 · 5% → 13회 · 10% → 3회`.
- 회피가 걸리면 `resolveHit` 이 ① 에서 조기 반환해 **`addRec` 를 부르지 않으므로 `recA` 가 0 으로 남는다.** 실측으로 확인했다 — 회피 강제 시 `recA=0`, 미회피 시 `recA=17`.
- 이 파일은 시드를 고정하지 않아(`board()` 에 `setSeed` 없음) 실행마다 draw 가 달라진다. 합산 확률 ≈ `0.325×0.05 + 0.075×0.10` ≈ **2.4%** — 로컬 반복에서는 계속 통과하고 CI 에서 터지기 딱 좋은 값이다.

즉 **제품 결함이 아니라 픽스처가 #233 이전 전제("공격하면 반드시 맞는다")를 그대로 들고 있던 것**이다.

### 수정 — 단언은 그대로, 픽스처만 결정론화

| 파일 | 절 | 조치 |
| --- | --- | --- |
| `smoke_turnflow.js` | D6 | 방어측 `dodge=0` 고정 |
| `smoke_turnflow.js` | G14~G20 | 양측 `dodge=0` 고정 (누가 선턴인지에 따라 공격 방향이 갈리므로 양쪽) |
| `smoke_fx_timing.js` | D | 속도·등급 동률 + 방어측 `dodge=0` |
| `smoke_turnflow_timers.js` | 3·7 | 속도·등급 동률 + 방어측 `dodge=0` |

`recA>0` · 순서 문구 · 시간 측정 등 **원래 단언은 한 글자도 바꾸지 않았고 약화·삭제하지 않았다.** 회피율 0 은 ① 의 `rand()` 호출 자체는 그대로 소비하므로 이웃 단언의 난수 정렬도 바뀌지 않는다. `smoke_shock`·`smoke_attack_balance` 가 같은 이유로 이미 쓰고 있는 조치와 같은 방식이다.

### 한 번의 실행이 드러낸 두 번째 같은 뿌리

수정 뒤 `smoke_turnflow.js` 를 1회 실행하자 이번에는 **G19**(기술 그룹 → 피해 그룹 순서)가 떨어졌다. 회피되면 "회피했다!" 만 나오고 피해 그룹이 아예 생기지 않는 **같은 뿌리**였다. 같은 방식으로 고쳤다.

### 정적 점검 — 같은 전제를 쓰는 다른 CI 픽스처

공격을 실제로 구동하는 회귀 14파일을 정적으로 훑어 "회피·선턴을 고정하지 않은 채 피해 발생을 단언하는" 곳을 찾았다.

- **이미 안전(회피 고정함)**: `smoke_attack_balance` · `smoke_cross_skill` · `smoke_issue146` · `smoke_issue233` · `smoke_search_packages` · `smoke_shock`
- **이미 안전(시드 고정)**: `smoke_minion_art` — `setup()` 이 `setSeed(20260907)` 을 걸어 draw 와 판정이 모두 결정론이다. J13 은 순서에 기대지만 시드로 고정돼 있어 손대지 않았다.
- **피해 단언 없음**: `smoke_online_sync` · `smoke_orientation_audit` · `smoke_own_side` · `smoke_public_rooms`(서버가 준 픽스처 값만 본다)
- **노출돼 있어 함께 고침**: `smoke_fx_timing`(D 절) · `smoke_turnflow_timers`(3·7 절) — 둘 다 시드가 없고, 선턴까지 무작위라 노출이 회피보다 훨씬 컸다. 실제로 `smoke_turnflow_timers` 7b 는 이 Issue 작업 초기 베이스라인에서 이미 한 번 떨어진 적이 있고 그때는 원인을 특정하지 못했는데, 이번에 같은 뿌리로 설명된다.

부수 효과로 이 파일들의 **잠복 불안정성이 제거**됐다.

### 검증 (정확히)

- `node demo/test/regression/smoke_turnflow.js` → **203 pass / 0 fail** (수정 후 1회).
- `node demo/test/regression/smoke_fx_timing.js` → **82 pass / 0 fail** (1회).
- `node demo/test/regression/smoke_turnflow_timers.js` → **36 pass / 0 fail** (1회).

뒤 두 건은 **이번에 편집한 파일이라 편집분을 한 번도 실행하지 않은 채 넘기지 않으려고 각 1회만** 돌렸다(반복 루프 아님, 전체 스위트 실행 아님). 나머지 회귀와 필수 CI 6종은 지시대로 PR CI 가 확인한다.

제품 코드(`demo/index.html`)는 **건드리지 않았다** — 이번 실패는 클라이언트 결함이 아니라 픽스처 전제 문제였다. 서버·Git·아트·지연 보고서에도 손대지 않았다.


---

## 11. 후속 — PR239 CI A `smoke_cross_skill` D10 (2026-09-16, `ctx_f0661d5f2be7`)

CI run 35068734897 / job 104704970948, head `4dcbcc1`, 85 pass 1 fail. **로컬에서도 재현됐다.**

- **근본 원인은 선턴이지 회피·치명·상태가 아니다.** 이 파일의 `openBattle` 은 이미 `dodge=0·crit=0` 을, `fixed()` 가 `dmgVar=0·statusProb=1` 을 고정하고 있었다. 남은 변수는 **속도**였다 — D10 의 공격자는 레거시 말(`skills=null·rosterId=null`)이라 스탯이 새 게임마다 무작위 배정되는 아키타입에서 오고, 실측 draw 는 `spd 6·8·10·14` 였다. 방어측 M-G1(std, spd 10)보다 느리면 선턴이 D 가 되어 `__actCore("skill")` 이 **D 로 실행**되고, `L.em` 은 맞지 않아 `hp===100-41`·`burn` 이 함께 떨어진다. 30회 표본에서 **12회(40%)** 가 D 선턴이었다.
- **수정**: `openBattle` 의 기존 고정 줄에 `d.spd=a.spd; d.grade=a.grade;` 를 더해 동률 → 접촉 개시자(A) 선턴으로 굳혔다. 이 파일이 "교차 속성 판정만 본다"며 이미 회피·치명을 0 으로 눕히던 자리와 같은 성격이라 한 곳에서 계열 전체를 없앤다. **피해 41·화상 단언은 그대로 두었고 약화·삭제하지 않았다.**
