...
| 미공개 말 공통 덮개 | 1 | 64 | 상대 미공개 말. **흰색 단색 + 알파**. CJ 가림 외형 요청을 문자 없는 덮개로 표현(구체 형태는 Earth 제작 선택) |
| 속성 아이콘 불·물·풀·번개 | 4 | 64 | 보드 정보행·배지·기술 버튼 접두 |
| 추측 메모 8종 | 0 | — | 위 토큰 4 + 속성 4를 **그대로 재사용** (신규 제작 금지 — 기호 어휘 통일) |
| 흔적 마커(🔍 대체) | 1 | 48 | 탐색 가능 칸 표시 |

후속 추가 범위: 공통 가림 **3D 메시1종**(FBX + 대체 OBJ/MTL), 편집용 Blender 원본·팀 틴트·보드 적합성 프리뷰.
이 모델은 위 PNG 수량에 합산하지 않으며, PNG 전용 루트 manifest와 분리된 `tokens/model-manifest.csv`로 관리한다.
미공개 종류별 변형을 만들지 않고 같은 모델만 사용한다. `visibleTo=false`인 말은 덮개도 표시하지 않는다.
...
- **승인 완료:** 왕·동료는 상징 문양. 사람형·성별·종족 캐릭터를 추가하지 않는다.
- **승인 완료:** 폭탄은 어울리는 형태로 제작하고, 함정은 물리적 덫으로 표현한다. 마법 함정 제안은 대체됐다.
- **승인 완료:** 아군/적군 파랑·빨강 틴트. 정체 미공개 때 쓰는 공통 가림 모델도 제작한다.
