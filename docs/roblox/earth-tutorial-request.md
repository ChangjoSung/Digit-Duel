...
|---|---|---|---|
| 1 | `tut_win` | 이기는 법 세 가지 | 왕을 잡는 장면 · 내 왕이 상대 끝줄(금색 띠)에 닿는 장면 · 상대 말이 전부 X 된 장면 — 세 칸으로 나눠 |
| 2 | `tut_pieces` | 말 5가지와 숨은 정체 | 👑 왕 · 🤝 동료 · ⚔ 하수인 · 💣 폭탄 · 🪤 함정 다섯 토큰. 상대 쪽은 전부 **?** 로 |
| 3 | `tut_move` | 움직이기와 숲에 숨기 | 상하좌우 1칸 화살표(대각선은 X) · 숲 칸의 반투명 말 · 옆에 오면 보이는 모습 |
| 4 | `tut_search` | 흔적과 탐색 | 숲 칸에 선 말 → 🔍 흔적 표시 → **다음 차례** 탐색 → 📦 선물 |
| 5 | `tut_battle` | 옆에 붙으면 꼭 싸워요 | 불🔥→풀🌿→번개⚡→물💧→불 상성 고리 · 두 말이 붙어 ⚔ 가 되는 모습 |
| 6 | `tut_bomb` | 폭탄과 함정 | 하수인+폭탄 = 둘 다 X · 동료/왕+폭탄 = 폭탄만 X · 폭탄+폭탄 = 둘 다 X · 함정은 고정(발 묶임) |
| 7 | `tut_capture` | 잡아오기와 도망치기 | HP 30% 아래 하수인에게 🔴 몬스터볼 → 성공 70% · 🏃 도망 30% |
| 8 | `tut_tele` | 텔레포트(순간이동) | 상대 진영에 들어간 내 말 ↔ 다른 내 말이 자리를 바꾸는 양방향 화살표 · 함정에 걸린 말은 X |
| 9 | `tut_burn` | 버닝 타임(불타는 시간) | 🔥 65턴부터 곧게 2칸 · 상대 숲·상대 땅에서는 1칸(X 표시) |
| 10 | `tut_check` | 빠른 복습 — 첫 차례 체크리스트 | 첫 차례에 할 일 3~4개를 아이콘 목록으로 (말 고르기 → 1칸 이동 → 붙으면 전투 → 턴 종료) |
...
- **첫 접속 시 자동으로 띄울 것인가.** HTML 은 `#128` 로 "문서 로드당 1회" 자동 표시다. Roblox 는 접속할 때마다 자동으로 띄우면 재접속·리스폰마다 성가실 수 있다. CJ 확인 전까지 **자동 표시 없이 간판·버튼으로만** 열게 할 계획이다.
- **간판을 로비 어디에 세울 것인가** — 스폰 지점 앞이 자연스럽지만 테이블 배치와 겹치지 않아야 한다. Mars 가 좌표를 제안하고 CJ 가 확인한다.
- 튜토리얼을 **대전 중에도** 열 수 있게 할 것인가 (지금 계획: 로비에서만).
