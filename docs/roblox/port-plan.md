...
| 4 | 운영: 재접속·**추측 메모**·DataStore 전적 | 🟡 부분 (2026-09-10) — 재접속·메모·전적 완료 · 관전·랭크·매치메이킹 고도화 대기 |
...
## Phase 4 (2026-09-10 CJ 지시로 착수한 범위)

| 기능 | 구현 | 비고 |
|---|---|---|
| **추측 메모** (#11·#36) | 클라 전용 `memos[pieceId]` · [📝 추측 메모] 토글 → 미공개 상대 말 클릭 → 8종(왕·동료·속성별 하수인·폭탄·함정) 선택 · 셀 왼쪽 아래 이모지 배지 | **서버로 보내지 않는다** — 상대는 알 수 없다. 말이 사라지면 자동 정리, 정체가 공개되면 실제 정체가 우선 |
| **재접속** | 대전 중 연결이 끊기면 즉시 기권이 아니라 **90초 유예**(`RECONNECT_GRACE`) — 자리를 잠가 두고 세션 보존, 같은 UserId 재접속 시 자리·세션 복구. 유예가 지나면 그때 기권 | 서버 인스턴스 안에서만 유지 (서버가 내려가면 소멸). 상대에게는 "🔌 상대 접속 끊김" 배너·대기 안내 |
| **전적 저장** | `src/server/Stats.luau` — DataStore `DigitDualStats_v1`, 키 `p_<UserId>`, 값 `{wins,losses,draws,games,updated}`. 접속 시 로드·대전 종료 시 기록·60초 주기 저장·퇴장/서버 종료 시 flush | DataStore 접근이 막히면(Studio 설정 미허용) **메모리 전용으로 조용히 낮춘다** — 저장 실패가 대전을 막지 않는다. 봇 대전은 기록하지 않는다. HUD·종료 화면에 "N승 N패 N무 · 승률 N%" 표시 |
| **게임 기록 창** | 사이드 패널은 최근 14줄만 보여 준다 — [📜 기록] 버튼으로 **전체 기록(최근 300줄)** 을 번호와 함께 스크롤해서 본다. 새 줄이 들어오면 열려 있는 창도 같이 갱신되고 최신 줄로 내려간다 | 내 화면 전용 · 서버로 아무것도 보내지 않는다. 게임 밖 로그(오류·서버 print)는 F9 개발자 콘솔에서 본다 |
| **전투 연출(FX)** | Earth 납품 스프라이트 13종(768×96 · 96×96 8프레임)을 전투 창(두 카드가 맞닿는 지점)과 중앙 배너 안에서 1회 재생. 판정은 `Banner.fxFromBattle` (테스트 [14]) · 배너는 tone 에 따라 배경까지 초록/빨강으로 바뀐다 | **표시 계층 전용 — 입력을 막지 않는다.** 프레임당 0.055초(≈0.44초). 입력 잠금·화면 흔들림·좌우 구분 연출은 [기획 필요] 유지 |
| **클라이언트 모듈 분리** | `src/client/` 를 LocalScript 본체 + 자식 ModuleScript 5개(`Ui`·`BannerUi`·`LogUi`·`MemoUi`·`RosterDetail`)로 나눴다. 옮긴 코드는 **원문 그대로**이고 동작 변경은 없다 | Luau 는 함수 하나가 지역 변수 200개까지 — 본체가 전부 최상위 스코프라 예산을 공유한다. 209 → **168개**. `rbxcheck.js` 가 170 초과 경고·190 초과 실패로 재발을 막는다 |
| 관전·랭크·매치메이킹 고도화 | 미착수 | 별도 요청 대기 |

## CJ 결정 기록 (Roblox 포팅 범위)

| 날짜 | 안건 | 결정 | 반영 위치 |
|---|---|---|---|
| 2026-09-10 | 잔류장(풀 속성) 대체 효과 — HTML 임시 구현이던 **흡수 회복 40%** 를 정식 규칙으로 볼 것인가 | **회복이 맞다** (CJ: "11번 풀속성은 회복이야 그게 맞아"). 풀은 대응되는 상태이상이 없어 자기 잔류장이 회복으로 작동한다. 수치 40% 는 기존 값 유지 | `roblox/src/shared/Battle.luau` 잔류장 분기 주석 · 이 표 · GDD-13 Decision Log 반영은 Mercury 소관 |
