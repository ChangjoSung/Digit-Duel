...
**P0-A 하수인 재출력 60장 완료, Saturn 기술·정적 시각 검수 PASS. 요청서 전체 P0 완료가 아니다.**
P0-B·C·D 69장과 P1·P2는 아직 납품하지 않았다. 요청서 8장의 미확정 사항은 결정하지 않았다.

| 묶음 | 요구 PNG 수 | 현재 납품 | 상태 |
|---|---:|---:|---|
| P0-A 하수인 20종 × 3용도 | 60 | 60 | 완료·독립 QA PASS |
| P0-B 토큰·속성·흔적 | 14 | 0 | 미납품·기획 결정 필요 |
| P0-C 타일·하이라이트·프레임 | 10 | 0 | 미착수 |
| P0-D UI·상태·아이템·행동·기술 종류 | 45 | 0 | 미착수 |
| **P0 합계** | **129** | **60** | **69장 남음** |
| P1 전투 배경·FX·배너 | 프레임 설계에 따라 변동 | 0 | 미착수 |
| P2 스토어·모바일 | 콘셉트·레이아웃 결정 필요 | 0 | 미착수 |

P0-D 행동 아이콘은 제목의 “9종”이 아니라 실제 열거된 10개와 경로 목록을 기준으로 수량을 셌다.
...
- PNG 60장 합계: **4,384,108 bytes**. [납품 manifest](../../../roblox/assets/manifest.csv) 60행과 파일 경로 집합이 정확히 같다.
...
아직 없는 P0-B 토큰 5종·속성 4종의 32px·흑백 검수, 9-slice 늘림 검수, Studio 실제 표시,
기기별 가독성, 업로드·rbxassetid 연결, 권리 체인 포괄 조사, 사람 대상 블라인드 식별 시험은 수행하지 않았다.

## [기획 필요] — 결정하지 않은 사항

1. 왕·동료: 상징형인지, 사람/비인간 캐릭터인지. **제안(미승인): 왕관·협력 문양의 상징형.**
2. 폭탄·함정: 마법 장치인지 기계 장치인지. **제안(미승인): 마법 장치형.**
3. 아군·적군: 파랑/빨강 틴트만 사용할지 별도 프레임 모양도 만들지. **제안(미승인): 기존 틴트 유지.**
...
6. 요청서 P0-B의 `unknown_64.png` “?”와 전 이미지 텍스트 금지의 충돌: “?”를 기호 예외로 허용할지, 코드 텍스트로 유지할지 확인 필요.
7. P2 썸네일1920×1080·모바일목업1080×1920과 “전 PNG≤1024” 조건의 적용 범위: 스토어/문서 예외인지 확인 필요.

첫 세 항목은 CJ에게 질문한 상태이며 무응답을 승인으로 간주하지 않았다.
...
아래 파일 경로는 공유 작업공간 `C:\WOOK\pvpserver` 기준이다.
...
worker_done:
  ...
  summary: |
    ...
    P0-B/C/D 69PNG와 P1/P2는 미납품. 코드 파일 작성·수정 0개.
  requests_to_mars: |
