...
- [v0.6.0 인덱스](docs/milestone/v0.6.0/README.md)와 [사전 분석](docs/milestone/v0.6.0/reports/Mercury/unity-port-analysis.md)이 준비 범위·의존성의 원본이다. CJ가 #183 CLI 준비 → #182 최소 프로젝트 → #183 로컬 MCP 실연결 → #182 본구성·Android 검증 순서를 승인했다. 현재 실행 상태와 설치 예외는 [실행 원장](docs/milestone/v0.6.0/issues/182/Mercury/execution.md)을 따른다. Codex 활성 계정과 Claude Code 모두 이 PC의 로컬 stdio MCP를 사용한다. 연결된 Android 기기에서 APK 설치·실행 검증을 통과했으며 #185 이후 게임·UI·서버 구현은 별도 범위다. 2026-09-11 CJ QA PASS로 #182·#183 CLOSED. 후속 [#195](https://github.com/ChangjoSung/Digit-Duel/issues/195)에서 `F. Unity 구조·순수 C#·MCP`를 추가했고 실제 PR 성공 후 Unity 트랙 보호에 필수 7개를 적용했다. main/dev/Roblox 트랙은 기존 6개다. F는 Core/Application 실제 컴파일·구조·MCP를 검사하며 Unity Editor 전체 컴파일·Edit/Play·APK CI는 아니다. [범위·재현·보호 증거](docs/milestone/v0.6.0/issues/195/Mercury/operations.md)를 따른다.
