...
4. **🔍 흔적과 탐색** — 숲 어딘가에 선물이 숨어 있습니다. 말이 그 칸에 서면 흔적이 뜨고, 그 말로 **다음 차례**에 탐색을 누르면 선물(몬스터볼 · 회복약 같은 아이템 · 다음 싸움 힘 버프 · 공용 하수인)을 받습니다. 탐색은 하수인·동료·왕만 할 수 있습니다. v0.4.4 개발판에서는 하수인이 다른 속성 공격기를 얻어 공격 슬롯 하나를 교체할 수도 있습니다.
...
| v0.4.2 | Public·Apache-2.0 · 온라인 안내 간소화 · Windows 원클릭 LAN 실행 | 완료 |
| **v0.4.3** | **하수인 20종 아트·말판 아이콘·HP·배치 및 전투 이미지** | **현재 릴리스** |
| v0.4.4 | 공용 하수인 아트·탐색 기술 교체·내 진영 하단·상대 턴 메모·공격형 및 감전 조정 | 구현·독립 QA 완료 / CJ 플레이 확인 대기 |
| v0.5.0 | Unity 포팅 기반 (Android 타깃) | 예정 |

<details>
<summary>버전별 변경 이력 보기</summary>

**v0.4.4 — 개발판 구현·독립 QA 완료**

- **포획 하수인 아트** — 기존 이미지 자산의 연결 누락을 수정했습니다. 숲 공용 하수인은 해당 속성 표준형, 전투에서 포획한 하수인은 원래 종의 전투 그림을 사용합니다.
- **탐색 기술 교체** — 하수인에게 다른 속성 공격기 한 개를 제안합니다. 두 공격 슬롯 중 하나를 교체하거나 유지할 수 있으며 남은 쿨타임은 이어집니다. 공격은 기술 속성, 방어는 본체 속성으로 판정하고 포획 시 장착 기술도 승계합니다.
- **시점·메모** — 온라인 양측 모두 자기 진영을 아래에서 보며 상대 턴에도 미공개 상대 말에 추측 메모를 남길 수 있습니다.
- **전투 수치** — 공격형 공격력 26→25·결정타 위력 44→40, 일반 감전 부여율 70→50%. 화상·약화 70%와 확정 감전 100%는 유지합니다. 사람 플레이의 균형 개선은 추가 확인이 필요합니다.
- [구현 계약](docs/v0.4.4-gameplay-spec.md) · [진행 상황](https://github.com/ChangjoSung/Digit-Duel/milestone/10). CJ 플레이 QA와 v0.4.4 출시는 아직 남아 있습니다.

**v0.4.3 — 현재 릴리스**

- **하수인 아트 적용** — 20종의 배치 설명 이미지와 전투 이미지, 말판용 32px 아이콘을 연결했습니다. 내 말과 공개된 상대 말은 같은 표시 규칙을 사용하며, 잔여 HP를 아래 한 줄에 표시합니다.
- **추측 메모 유지** — 미공개 상대는 물음표 또는 내가 지정한 8종 메모로 표시합니다. 이미지가 없거나 로드에 실패하면 같은 자리에서 대체 표시합니다.
- **검증·출시** — 자동 회귀 571/571 및 Saturn 독립 QA 후 CJ 플레이 QA를 통과했습니다. [릴리스 v0.4.3](https://github.com/ChangjoSung/Digit-Duel/releases/tag/v0.4.3), main `cc3f382`.
- 탐색 공용 하수인 대리 출전의 이미지 누락은 [#91](https://github.com/ChangjoSung/Digit-Duel/issues/91)에서 후속 수정합니다.
