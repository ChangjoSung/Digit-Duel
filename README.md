...
두 사람이 같은 주소로 접속해 같은 코드를 입력하면 자동으로 서로 매칭됩니다.
...
> `127.0.0.1`은 서버를 실행한 PC 자신을 가리키는 주소입니다. 다른 사람은 이 주소와 코드를 받아도 접속할 수 없습니다.
...
2. 서버 화면에 표시된 두 가지를 상대에게 전달합니다.
   - 내부망 접속 주소: `http://192.168.x.x:8080`
   - 이번 실행의 접속 코드
3. 두 사람 모두 전달받은 내부망 주소를 브라우저에서 엽니다.
