...
로비 스폰 → 테이블 의자 앞에서 **[E] 앉기** (카메라가 판을 향함) → 하단 패널에서 **로스터 20종 중 6종 선택**(카드 클릭 → 초상화·기술 상세) → **[배치로 →]** 트레이의 말을 골라 **판의 내 진영 3행 칸을 클릭**해 14개 배치 (회수·남은 말 무작위·전체 회수, 급하면 [⚡ 전부 무작위]) → **[배치 완료 ✔]** → 맞은편에 상대가 앉아 배치를 마치면 그 둘만 대전 → **판 위의 내 말을 클릭하면 이동 가능 칸(파랑)·공격 가능 대상(빨강)이 표시**되고 칸을 클릭해 진행 (오른쪽 패널: 탐색·텔레포트·회복·턴 종료 — 가능할 때만 활성, 전투는 중앙 패널) → 종료 후 [닫기] → 앉은 채 재대전. 대전 중 자리 이탈 = 기권.
- 판 위 말은 **각 클라이언트가 서버 필터 뷰로 로컬 생성**한다 — 공개 말 = 도트 빌보드, 미공개 말 = 공통 덮개 메시(팀 틴트). 상대 클라이언트에는 아무 인스턴스도 복제되지 않는다.
- 판 파트가 없거나 HUD [2D 보드] 를 켜면 2D 보드 GUI 로 폴백 (타일·하이라이트·프레임 아트 적용).
- UI 아트(P0 129 PNG): 패널·버튼 4상태·배지·HP/방어막 바·상태/아이템/행동/기술 종류/속성/흔적 아이콘 전량 연결 — 계약은 `roblox/assets/ui/README.md`, 적용 위치는 `docs/roblox/art-apply-report.md` 3b.
