...
2026-09-10 · Ref #118. **기본 요청 P0 129/129 PNG 완료 + 별도 Phase3 AI 창·봇 추가 납품**.
...
- [manifest.csv](manifest.csv): 기본 P0 PNG129개 + AI UI6개 =135행. 기존18열 유지, 파일·원고 SHA-256와 크기·틴트·SliceCenter 기록. 모델은 넣지 않는다.
...
- 별도 [로비 납품](lobby/README.md): 기존6메시·6알베도·1상태 표지 + 신규봇1메시·1알베도, [로비 manifest](lobby/manifest.csv)15행. 루트135PNG와 분리한다.
...
현행 업로더의 세 manifest 집계는151대상(Image143·Model8)이며, 이번 dry-run은 **신규 Image7·Model1 · 기존143 재사용**이다.
기존 업로드는 Mars의 작업이다. Earth는 이번 AI 창·봇 자산을 업로드하거나 rbxassetid를 발급하지 않았다.
